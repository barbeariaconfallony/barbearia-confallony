...
interface FavoritosChartProps {
  ...
  profissionaisFavoritos: { nome: string; quantidade: number }[];
}
...
export const FavoritosChart = ({ 
  ...
  const chartData = useMemo(() => {
    ...
    const atendimentosAnoAtual = atendimentos.filter(atendimento =>
      isWithinInterval(atendimento.data_atendimento, { start: inicioAno, end: fimAno })
    );
    
    // Cria array com todos os meses do ano
    const mesesDoAno = eachMonthOfInterval({ start: inicioAno, end: fimAno });
    ...
    const dados = mesesDoAno.map(mes => {
      ...
      const dadosMes: any = { mes: nomeMes };
      
      // Conta serviços favoritos para este mês
      servicosFavoritos.forEach(servico => {
        const contagem = atendimentosAnoAtual.filter(atendimento =>
          isWithinInterval(atendimento.data_atendimento, { start: inicioMes, end: fimMes }) &&
          atendimento.servico_nome === servico.nome
        ).length;
      ...
      profissionaisFavoritos.forEach(profissional => {
        const contagem = atendimentosAnoAtual.filter(atendimento =>
          isWithinInterval(atendimento.data_atendimento, { start: inicioMes, end: fimMes }) &&
          atendimento.funcionario_nome === profissional.nome
        ).length;
  ...
  return (
    <div className="w-full h-full">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-base sm:text-lg font-semibold">Serviços e Profissionais Favoritos</h3>
        <p className="text-xs text-muted-foreground ml-auto">Ano {new Date().getFullYear()} por mês</p>
      </div>
      
      {servicosFavoritos.length === 0 && profissionaisFavoritos.length === 0 ? (
            ...
            >
              <defs>
                {servicosFavoritos.map((servico, index) => (
                  <linearGradient key={`gradient-servico-${servico.nome}`} id={`gradient-servico-${index}`} x1="0" y1="0" x2="0" y2="1">
                ...
                {profissionaisFavoritos.map((profissional, index) => (
                  <linearGradient key={`gradient-profissional-${profissional.nome}`} id={`gradient-profissional-${index}`} x1="0" y1="0" x2="0" y2="1">
              ...
              <Tooltip 
                ...
                labelStyle={{ color: 'hsl(var(--foreground))', marginBottom: '8px', fontWeight: 500 }}
              ...
              {servicosFavoritos.map((servico, index) => (
                <Area
                  ...
                  type="monotone"
                  dataKey={servico.nome}
                  stroke={coresServicos[index % coresServicos.length]}
                  strokeWidth={2}
                  fill={`url(#gradient-servico-${index})`}
                  name={servico.nome}
                />
              ))}
              ...
              {profissionaisFavoritos.map((profissional, index) => (
                <Area
                  key={`profissional-${profissional.nome}`}
                  type="monotone"
                  dataKey={profissional.nome}
                  stroke={coresProfissionais[index % coresProfissionais.length]}
                  strokeWidth={2}
                  strokeDasharray="5 5"
                  fill={`url(#gradient-profissional-${index})`}
